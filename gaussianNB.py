...
data = ssc.socketTextStream("localhost", 7000)
...
def gaussianNB(X,y,gnb):
    if (os.path.isfile('gaussianNB.joblib')):
        gnb = load('gaussianNB.joblib')  
    gnb.partial_fit(X, y,classes=[1,0])
...
def func(rdd):
    ...
    if row:
        ...
        gaussianNB(X_train,Y_train,gnb)
    
data.foreachRDD(lambda rdd: func(rdd))
