...
km = MiniBatchKMeans(n_clusters=2, init='k-means++', n_init=1, init_size=250, batch_size=250, )

def kmeanstrain(X,y,km):
    if (os.path.isfile('km.joblib')):
        km = load('km.joblib')  
    km.partial_fit(X)
    dump(km, 'km.joblib')
...
def batch(rdd):
    ...
    if row:
        ...
        kmeanstrain(X_train,Y_train,km)
