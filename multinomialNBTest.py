...
def multinomialNBtest(X,y):
    gnb = load('multinomialNB.joblib') 
...
def preProcess(df):
    ...
    hashingTF = HashingTF(inputCol="processed", outputCol="rawFeatures", numFeatures=20)
    df = hashingTF.transform(df)
    idf = IDF(inputCol="rawFeatures", outputCol="counts")
    idfModel = idf.fit(df)
...
def batch(rdd):
    ...
    if row:
        ...
        multinomialNBtest(X_test,Y_test)
